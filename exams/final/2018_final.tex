...
\begin{enumerate}
    ...
    \item Find \( \vb{x}(t) \) for \( \vb{x}(0) = \begin{bmatrix} 1 & 0 & 2 \end{bmatrix} ^T\).
\end{enumerate}
\clearpage
\newpage
\mbox{}
...
Consider the translational mechanical system defined by the following differential equations
\begin{align*}
    2 \ddot{x_1} + 10 x_1 - 6 x_2 &= 0 \\
    -6 x_1 + 5 \ddot{x}_2 + 7 x_2 + 4 x_2 + 4 \dot{x}_2 - 4 \dot{x}_3 &=0 \\
    -4 \dot{x}_2 + 7 \ddot{x}_3 + 5 \dot{x}_3 &= 0.
\end{align*}

\begin{enumerate}
    \item Draw a mechanical model for this system.
    % \item Replace \( \dot{x}_1 \) with \( i_1 \), \( \dot{x}_2\) with \( i_2 \), and \( \dot{x}_3\) with \( i_3\) where \( i \) represents the current within a loop.
    %     Draw an equivalent electrical circuit for this system.
\end{enumerate}
\clearpage\newpage
\mbox{}
\clearpage
